  ...
  return (
    <a href={project.link} target="_blank" rel="noopener noreferrer">
      <div
        ...
        className={
          isMobile
            ? `group relative w-screen overflow-hidden bg-neutral-200`
            : `group relative h-screen w-screen overflow-hidden bg-neutral-200`
        }
      >
        ...
        <div className={`${styles.projectDescription} z-10`}>
          <span>
            0{project.index + 1}-0{project.totalProjects}
          </span>
          <p className={`${styles.title} uppercase`}>{project.title}</p>
        </div>
      </div>
    </a>
  );
